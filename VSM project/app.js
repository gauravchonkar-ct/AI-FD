...
function renderFlowchart(data) {
    ...
    try {
        mermaid.render('mermaid-svg', mermaidSyntax).then(result => {
            mermaidDiagram.innerHTML = result.svg;
        });
    } catch (error) {
        console.error('Mermaid rendering error:', error);
        mermaidDiagram.innerHTML = '<div class="status-message status-error">Error rendering flowchart</div>';
    }
}
...
/**
 * Generate Mermaid syntax from VSM data
 */
function generateMermaidSyntax(data) {
    let syntax = 'flowchart TD\n';

    // Create a map of step names to safe IDs
    const stepIdMap = {};
    data.steps.forEach((step, index) => {
        const safeId = `S${index + 1}`;
        stepIdMap[step.name] = safeId;
    });

    // Define nodes with safe IDs - compact size
    data.steps.forEach((step, index) => {
        const safeId = `S${index + 1}`;
        const timeInfo = step.processTimeMins > 0 ? `${step.processTimeMins}m` : '';
        
        // Very short step names
        const shortName = (step.name || '').substring(0, 10);

        // Classify step type for color coding
        const stepType = classifyStepType(step);

        // Minimal node content
        syntax += `    ${safeId}["${shortName}"]:::${stepType}\n`;
    });

    syntax += '\n';

    // Define connections - sequential flow
    for (let i = 0; i < data.steps.length - 1; i++) {
        const currentId = `S${i + 1}`;
        const nextId = `S${i + 2}`;
        syntax += `    ${currentId} --> ${nextId}\n`;
    }

    // Define class styles with smaller sizing
    syntax += '\n    classDef VA fill:#10B981,stroke:#059669,stroke-width:1px,color:#fff\n';
    syntax += '    classDef ENVA fill:#F59E0B,stroke:#D97706,stroke-width:1px,color:#fff\n';
    syntax += '    classDef NVA fill:#EF4444,stroke:#DC2626,stroke-width:1px,color:#fff\n';
...
async function downloadFlowchartPDF() {
    try {
        ...
        const flowchartElement = document.querySelector('#mermaidDiagram svg');
        if (!flowchartElement) {
